		...
		void *data_buffer, size_t burst_payload_length)
{
	...
	if ((burst_payload_length - RLE_START_HEADER_SIZE) <= 0) {
		printf("ERROR %s:%s:%d: Available burst payload size [%d]"
	...
	zc_rle_header_start *rle_s_hdr = (zc_rle_header_start *)rle_ctx->buf;
	...
	rle_s_hdr->header.head.b.proto_type = rle_ctx_get_proto_type(rle_ctx);
		...
		int type_rle_frag)
{
	...
	/* map RLE header to the already allocated buffer */
	zc_rle_header_cont_end *rle_c_e_hdr =
		(zc_rle_header_cont_end *)rle_ctx_get_end_address(rle_ctx);
		...
		int type_rle_frag)
{
	...
	switch (type_rle_frag) {
		case RLE_PDU_START_FRAG:
			ret = add_start_header(rle_ctx, burst_payload_length);
			break;
		case RLE_PDU_CONT_FRAG:
		case RLE_PDU_END_FRAG:
			ret = add_cont_end_header(rle_ctx, burst_payload_length, type_rle_frag);
			break;
		default:
...
{
	/* clear old data */
	zc_rle_header_start *rle_hdr = (zc_rle_header_start *)rle_ctx->buf;
	memset((void *)rle_hdr, 0, sizeof(zc_rle_header_start));
