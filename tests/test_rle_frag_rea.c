/**
 * @file   test_rle_frag_rea.c
 * @brief  Lib RLE fragmentation/reassembly functions
...
static int run_test_frag_rea(char *pcap_file_name, uint32_t param_ptype, uint16_t param_bsize,
                             int nb_fragment_id,
                             int trailer_mode);
...
/*        while(((packet = (unsigned char *)pcap_next(handle, &header)) != NULL) && nb_frag_id < nb_fragment_id)*/
	while (((packet = (unsigned char *)pcap_next(handle, &header)) != NULL)) {
		...
		for (;; ) {
			...
			if (opt_verbose_flag) {
				PRINT(
				        "INFO: Remaining PDU size to send = [%5d] burst size = [%4d]\n",
